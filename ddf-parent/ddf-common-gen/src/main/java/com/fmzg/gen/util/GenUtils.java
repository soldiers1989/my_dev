...
import com.fmzg.gen.entity.GenCategory;
...
import com.fmzg.gen.mapper.JaxbMapper;
...
public class GenUtils {
	...
	public static void initColumnField(GenTable genTable){
		for (GenTableColumn column : genTable.getColumnList()){
			...
			// 设置java类型
			if(column.getComments().contains("枚举类型")) {
				column.setJavaType(genTable.getClassName()+StringUtils.toCapitalizeCamelCase(column.getName()));
			}else if (StringUtils.startsWithIgnoreCase(column.getJdbcType(), "CHAR")
				...
				column.setJavaType("Boolean");
					...
					|| column.getJdbcType().toLowerCase().contains("INT".toLowerCase())){
				// 如果是浮点型
...
//		}
	...
	public static Map<String, Object> getDataModel(GenScheme genScheme){
		...
		model.put("basePackageName", StringUtils.lowerCase(genScheme.getBasePackageName()));
		model.put("bizPackageName", StringUtils.lowerCase(genScheme.getBizPackageName()));
		...
		model.put("functionAuthor", StringUtils.isNotBlank(genScheme.getFunctionAuthor())?genScheme.getFunctionAuthor():"hujun");
	...
	public static String generateToFile(GenTemplate tpl, Map<String, Object> model, boolean isReplaceFile){
		// 获取生成文件
		String fileName = Global.getProjectPath() + File.separator + tpl.getName()
